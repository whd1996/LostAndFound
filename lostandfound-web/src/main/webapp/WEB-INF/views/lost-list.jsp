   ...
   <div id="pagewrapper" style="width:100%;margin:0 auto;overflow:visible;">
	...
	<span style="width:38%;float:right;text-align:right;padding-right:10%;">
		<select name="type"	id="types" style="width:160px;">			
			...
			<option value="${path}/lost/lost-list.html/1/${pageSize}/card">卡类证件</option>
			<option value="${path}/lost/lost-list.html/1/${pageSize}/book">书籍资料</option>
			<option value="${path}/lost/lost-list.html/1/${pageSize}/clo">衣物服饰</option>
			<option value="${path}/lost/lost-list.html/1/${pageSize}/per_eff">随身物件</option>
			<option value="${path}/lost/lost-list.html/1/${pageSize}/elec">电子数码</option>
			<option value="${path}/lost/lost-list.html/1/${pageSize}/veh">交通工具</option>
			<option value="${path}/lost/lost-list.html/1/${pageSize}/other">其他宝贝</option>
		</select>
	</span>
	<script>
		//按类搜索
	 ...
	 <!--失物信息内容展示容器-->
	  ...
	  <c:otherwise>
	  <tbody>	  
	  <c:forEach items="${lostThingsList}" var="lostList" varStatus="vs">
		<tr
			<c:if test="${lostList.status ==0}">
      ...
      <div style="width:100%;height:50px;text-align:center;margin:0 auto;padding-bottom:10px;">		
		...
		totalPage="${totalPage}" currentPage="${pageNum}" url="${path}/lost/lost-list.html"
