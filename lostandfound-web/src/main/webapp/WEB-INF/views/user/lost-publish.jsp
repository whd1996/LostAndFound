...
<html>
<head>
	<title> ${webSiteTitle} 发寻物贴页面 </title> 
	...
	function checkVaildForm(form){
		...
		if(form.lostTime.value.trim() == ""){
			// alert("时间要选一下嘛!");
			form.lostTime.focus();
			valid(form.lostTime, "丢失时间");
			return false;
		}
...
<body style="background: rgb(255, 255, 255); overflow-x: hidden;"> 
   ...
   <div id="pagewrapper" style="margin:0 auto;height:auto;overflow:visible;min-height:800px;*height:800px;border: 1px solid white;">
	...
	<!-- 导航条 end -->
	                         ...
	                         <!--------------页眉结束------------------>				  
	                         <!-------------主体部分开始--------------->
     <div id="details" class="container-fluid" style="min-height:520px;border: 1px solid red;"> 
	...
	<hr style="width: 80%; margin:15px auto;"> 
  	...
  	<form action="${path}/lost/lost-publish.html" method="post" enctype="multipart/form-data" onsubmit="return checkVaildForm(this)" >
	 <table style="width: 100%;margin: 0px auto;background-color: #f5f5f5;">
	  ...
	  <tr style="height:30px;">
	   ...
	   <input type="hidden" name="token" value="${token}" />
	   	<input type="text" name="thingsName" id="thingsName" value="${lostThings.thingsName}" onfocus="valueListenerLimiter(this, 12);" onblur="labelHidden(this);" style="width: 180px;margin-top:10px;" maxlength="12">
	  ...
	  <tr style="height:30px;">
	   <td id="date">丢失时间 : </td>
	   <td id="date">
	   <input class="Wdate" type="text" name="lostTime" value="<fmt:formatDate value="${lostThings.lostTime}" pattern="yyyy-M-d" />" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd', maxDate:'%y-%M-%d'})" placeholder="选择日期" readonly style="width: 120px;margin-top:10px;"/>
	  </td>
	  </tr>
	  <tr>
	   <td id="place">丢失地点 : </td>
	  ...
	  <tr>	 
	   ...
	   <td>
		...
		<script>
		   ...
		   var tType = "${lostThings.thingsType}";
	  ...
	  <tr>
	   <td>相关描述 : </td>
	   <td>
	  	<textarea name="thingsDes" id="thingsDes" rows="3" cols="20" onfocus="valueListenerLimiter(this, 50);" onblur="labelHidden(this);"style="width: 300px;max-width:350px;max-height: 80px;margin-top:10px;text-align: left;">${lostThings.thingsDes}</textarea>
	   	<span style="color:blue;font-size:12px;"></span>
	   </td>
	  </tr>	 
	  <tr>
	  ...
	  <tr style="text-align: center;">
	   ...
	   <input type="submit" class="btn-success" id="publish" name="publish" value="发帖寻物" style="margin-top: 20px;">
	   </td>
	  </tr>	    
	 </tbody>
	 </table>
