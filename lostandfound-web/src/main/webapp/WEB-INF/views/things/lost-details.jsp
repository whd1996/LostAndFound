...
<html>
<head>
	<title> ${webSiteTitle} 寻物贴详情页面 </title>
   ...
   <div id="pagewrapper" style="margin:0 auto;height:auto;overflow:visible;min-height:600px;*height:800px;border: 1px solid white;">
	...
	<script type="text/javascript">
		jQuery("#next").click(function (){
			...
			var url = "${path}/lost/lost-details.html/next/"+id;
		...
		jQuery("#prev").click(function (){
			...
			var id = ${lostThings.id};
			var url = "${path}/lost/lost-details.html/previous/"+id;
			jQuery.post(url, function (data) {
				if (data[0] != null) {
					var newId = data[0].id;
					var locationurl = location.href.substring(0, location.href.lastIndexOf("/") + 1);
  ...
  <div id="telInfo" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" width="300">
    <div class="modal-header">
       ...
       <c:if test="${user.id eq lostThings.uid}">
    ...
    <div class="modal-footer">
       ...
       <c:choose>
       <c:when test="${(user.id eq lostThings.uid) and (lostThings.status != 1)}">
       	<button class="btn btn-primary" onclick="confirm('${path}/user/lost-change.do','${lostThings.id}');">确定已认领</button>
       </c:when>
       <c:when test="${(user.id ne lostThings.uid) and (lostThings.status != 1)}">
       	<button class="btn btn-primary" onclick="confirm('${path}/user/lost-change.do','${lostThings.id}');">确定归还</button>
       </c:when>
       </c:choose>
    </div>
  </div>
 ...
 <table id="thingsinfo" style="width: 80%;margin: 0px auto;border: 1px solid red;">
	 ...
	 <c:when test="${!empty lostThings}">
	  <tbody>
	  <tr> <td>物品名称 : ${lostThings.thingsName}</td> </tr>
	  <tr> <td id="date">丢失时间 : ${lostThings.lostTime}</td> </tr>
	  <tr> <td id="place">丢失地点 : ${lostThings.lostPlace}</td> </tr>
	  <tr>	 
	   <td>所属类型 : <script>document.write(parseValue("${lostThings.thingsType}"));</script></td>
	   <c:if test="${lostThings.thingsType eq 'card' and !empty lostThings.thingsNo}">
	   <tr> <td>卡号 : ${lostThings.thingsNo}（安全起见，只为您显示后三位!）</td> </tr>
	   </c:if>
	  </tr>
	  <tr> <td>相关描述 : ${lostThings.thingsDes} </td> </tr>	 
	  <tr> <td><input type="button" class="btn-success" id="showTel" name="showTel" value="查看发帖人及其联系方式"> </tr>
	  <c:if test="${!empty lostThings.thingsImg}">
	  <tr>  
	  <td>物品图片: <img src="${path}/${lostThings.thingsImg}" alt="物品图片" width="150" height="150" oncontextmenu="return false;" ondragstart="return false;" style="width:150px;height:150px;"> </td> 
	  </tr>	   	  
	  </c:if>   
	 </tbody>
	 </c:when>
	...
	<div style="margin-right:100px;"> 
	    <c:choose>
		<c:when test="${(lostThings.uid == user.id) and (lostThings.status != 1)}">
			<a href="javascript:void(0);" onclick="showMsg();" class="btn-success pull-right">宝贝已找到</a>
		</c:when>
		<c:when test="${lostThings.status == 1}">
